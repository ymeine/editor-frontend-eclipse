...
public class SpecificRuleBasedScanner extends RuleBasedScanner {
	...
	public IToken nextToken() {
		...
		RichToken enhancedToken = (temporaryToken instanceof RichToken) ? (RichToken) temporaryToken : null;
	...
	public int getTokenOffset() {
		...
		int offset = -1;
		if (this.currentToken instanceof RichToken) {
			RichToken enhancedToken = (RichToken) this.currentToken;
	 ...
	 * @return
	 */
	public RichToken getToken(boolean stopAtDefault) {
		RichToken containerToken = this.tokenStore.getToken(TokensStore.CONTAINER);
		while (this.fOffset < this.fRangeEnd) {
			...
			RichToken pocNext = (RichToken) next;
			RichToken pocNextClone = pocNext.clone();
			if (pocNextClone.getLength() == RichToken.UNDEFINED_INT && pocNextClone.getOffset() == RichToken.UNDEFINED_INT) {
