...
public class FunctionRule extends ContainerRule {
	...
	public IToken evaluate(ICharacterScanner initialScanner) {
		...
		this.addToken(this.tokenStore.getToken(TokensStore.FUNCTION, this.start, 8));
	...
	private boolean parseFunctionArguments() {
		...
		while (next != ICharacterScanner.EOF && charNext != ')') {
			...
			charNext = (char) next;
			if (charNext == '(') {
				this.addToken(this.tokenStore.getToken(TokensStore.FUNCTION, this.start + 8, this.offset - 7));
		...
		if (next == ICharacterScanner.EOF) {
			...
			if (argsOffset < this.offset) {
				this.addToken(this.tokenStore.getToken(TokensStore.DEFAULT, this.start + argsOffset, this.offset - argsOffset));
			}
			return false;
		} else {
			if (argsOffset < this.offset) {
			...
			this.addToken(this.tokenStore.getToken(TokensStore.FUNCTION, this.start + this.offset, 1));
		}
		return true;
	}
	...
	private void parseFunctionBody() {
		...
		if (next == ICharacterScanner.EOF) {
			this.unread();
			this.addToken(this.tokenStore.getToken(TokensStore.FUNCTION, initialOffset, this.offset - initialOffset));
			return;
		} else {
			this.addToken(this.tokenStore.getToken(TokensStore.FUNCTION, initialOffset, this.offset - initialOffset));
			bracketsCount++;
		...
		while (next != ICharacterScanner.EOF && bracketsCount > 0) {
			...
			if (tokenizedLentgh > 0) {
				...
				if (bracketsCount == 0) {
					...
					this.addToken(this.tokenStore.getToken(TokensStore.DEFAULT, this.start + this.offset, 1));
				}
			}
		}
		if (next == ICharacterScanner.EOF) {
